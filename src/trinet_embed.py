...
weight_fname = '../models/trinet-mars.npz'
...
def build_residual_block(incoming_layer,
    ...
    net_tmp, last_layer_name = build_simple_block(
        incoming_layer,
        list(map(lambda s: s % (ix, 2, 'a'), simple_block_name_pattern)),
...
def build_model(input_size):
    ...
    for char in block_size:
        if char == 'a':
            sub_net, parent_layer_name = build_residual_block(
                net[parent_layer_name], 1.0/2, 1.0/2, True, 4, ix='4%s' % char)
        else:
            sub_net, parent_layer_name = build_residual_block(net[parent_layer_name],
                                                              1.0/4,
                                                              1,
    ...
    for char in block_size:
        if char == 'a':
            sub_net, parent_layer_name = build_residual_block(
                net[parent_layer_name], 1.0/2, 1.0/2, True, 4, ix='5%s' % char)
        else:
            sub_net, parent_layer_name = build_residual_block(net[parent_layer_name],
                                                              1.0/4,
                                                              1,
...
# We subtract the per-channel mean of the "mean image"
# as loaded from the original ResNet-50 weight dump.
...
class BodyExtractor:
    '''
    Body Extractor Class
    '''
    def __init__(self):
        # Setup the original network
        self.resnet = build_model(input_size=(None, 3, 256, 128))
        self.im_mean = np.asarray([103.0626238, 115.90288257, 123.15163084], dtype=np.float32)

        # Now we modify the network's final pooling layer
        # and add 2 new layers at the end to predict the 128-dimensional embedding.
        # Different input size.
        self.inp = self.resnet['input']

        self.network_features = self.resnet['pool5']
        self.network_features.pool_size = (8, 4)

        #New additional final layer
        self.network = lasagne.layers.batch_norm(lasagne.layers.DenseLayer(
        ...
        self.network_out = lasagne.layers.DenseLayer(
            self.network,
            num_units=128,
            nonlinearity=None,
        ...
        with np.load(weight_fname) as f:
            ...
            lasagne.layers.set_all_param_values(self.network_out, self.param_values)


        #Setup the function to predict the embeddings.
        self.predict_features = theano.function(
            inputs=[self.inp.input_var],
            outputs=lasagne.layers.get_output(self.network_out, deterministic=True))

    def extract_feature(self, image):
        '''
        preprocess and extract feature
        '''
        img = cv2.resize(image, (144, 288))
        batch = get_augmentation_batch(img, self.im_mean)
        embedding = np.mean(self.predict_features(batch), axis=0)
        return embedding
...
#     print(count)
#     label = get_parent_dir_name(image_path)
#     image = cv2.imread(image_path)
#     resize_image = cv2.resize(image, (144, 288))
#     batch = get_augmentation_batch(resize_image, im_mean)
#     embedding = np.mean(predict_features(batch), axis=0)
#     labels.append(label)
...
# pickle.dump(data, open('trinet.pickle', 'wb'))
