...
class MTCNNDetector(object):
    ...
    def detect_face(self, img, minsize=Config.MTCNN.MIN_FACE_SIZE):
        ...
        if numbox > 0:
            ...
            points[0:5, :] = (np.tile(w, (5, 1)) *
                              points[0:5, :] +
                              np.tile(total_boxes[:, 0], (5, 1)) -
                              1)
            points[5:10, :] = (np.tile(h, (5, 1)) *
                               points[5:10, :] +
...
class CVDetector(object):
    
    def __init__(self,
                 xml_file=Config.HAAR_FRONTAL,
                 scale_factor=1.1,
                 min_neighbors=5):
        self.haar_face_cascade = cv2.CascadeClassifier(xml_file)
        self.scale_factor = scale_factor
    ...
    def detect_face(self, img):
        ...
        bbs = self.haar_face_cascade.detectMultiScale(gray, scaleFactor=self.scale_factor, minNeighbors=self.min_neighbors)
        bbs = [[bb[0], bb[1], bb[0] + bb[2], bb[1]+bb[3]] for bb in bbs]
        return bbs
...
class DLIBDetector(object):
    ...
    def detect_face(self, img):
        ...
        bbs = [[det.left(), det.top(), det.right(), det.bottom()] for det in dets]
        return bbs
